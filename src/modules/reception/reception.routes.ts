...
receptionRouter.post("/customers", async (req, res) => { const data = customerSchema.safeParse(req.body); if (!data.success) { invalid(res, "customer", data.error.flatten()); return; } res.status(201).json({ customer: await prisma.customer.create({ data: { tenantId: tenantId(req), ...data.data } }) }); });
receptionRouter.patch("/customers/:id", async (req, res) => { const data = customerSchema.partial().safeParse(req.body); if (!data.success) { invalid(res, "customer", data.error.flatten()); return; } const customer = await prisma.customer.updateMany({ where: { id: req.params.id, tenantId: tenantId(req) }, data: data.data }); if (!customer.count) { res.status(404).json({ error: "Customer not found" }); return; } res.json({ customer: await prisma.customer.findUniqueOrThrow({ where: { id: req.params.id } }) }); });
...
receptionRouter.get("/rooms", async (req, res) => res.json({ rooms: await prisma.room.findMany({ where: { tenantId: tenantId(req) }, orderBy: { number: "asc" } }) }));
receptionRouter.post("/rooms", async (req, res) => { const data = roomSchema.safeParse(req.body); if (!data.success) { invalid(res, "room", data.error.flatten()); return; } res.status(201).json({ room: await prisma.room.create({ data: { tenantId: tenantId(req), ...data.data } }) }); });
receptionRouter.patch("/rooms/:id", async (req, res) => { const data = roomSchema.partial().safeParse(req.body); if (!data.success) { invalid(res, "room", data.error.flatten()); return; } const room = await prisma.room.updateMany({ where: { id: req.params.id, tenantId: tenantId(req) }, data: data.data }); if (!room.count) { res.status(404).json({ error: "Room not found" }); return; } res.json({ room: await prisma.room.findUniqueOrThrow({ where: { id: req.params.id } }) }); });
receptionRouter.delete("/rooms/:id", async (req, res) => { const room = await prisma.room.deleteMany({ where: { id: req.params.id, tenantId: tenantId(req) } }); if (!room.count) { res.status(404).json({ error: "Room not found" }); return; } res.status(204).send(); });

async function roomIsAvailable(scope: { tenantId: string; roomId: string; checkIn: Date; checkOut: Date; excludeId?: string }) {
  return !(await prisma.reservation.findFirst({ where: { tenantId: scope.tenantId, roomId: scope.roomId, status: { in: ["BOOKED", "CHECKED_IN"] }, checkIn: { lt: scope.checkOut }, checkOut: { gt: scope.checkIn }, ...(scope.excludeId ? { id: { not: scope.excludeId } } : {}) } }));
}
