...
storeRouter.get("/", async (req, res) => res.json({ stores: await prisma.store.findMany({ where: { tenantId: tenantId(req) }, include: { _count: { select: { items: true } } }, orderBy: { name: "asc" } }) }));
...
storeRouter.get("/:id/movements", async (req, res) => { const store = await prisma.store.findFirst({ where: { id: req.params.id, tenantId: tenantId(req) } }); if (!store) { res.status(404).json({ error: "Store not found" }); return; } const movements = await prisma.inventoryMovement.findMany({ where: { storeId: store.id, tenantId: store.tenantId }, include: { inventoryItem: { select: { id: true, name: true, sku: true, unit: true } } }, orderBy: { occurredAt: "desc" } }); res.json({ store, movements }); });
